...
def calculate_irr(
    ...
    try:
        irr = optimize.newton(
            npv_equation, guess, tol=tolerance, maxiter=max_iterations
        )
        return irr
    except RuntimeError:
...
def calculate_payback_period(
    cash_flows: List[float], cumulative: bool = True
) -> float:
    """
...
def calculate_roi(
    ...
    if investment is None:
        if cash_flows[0] < 0:
            ...
            raise ValueError(
                "Initial investment not provided and first cash flow is positive"
            )
...
def create_loan_amortization_schedule(
    principal: float,
...
def calculate_depreciation(
    ...
    double_declining_factor: float = 2.0,
) -> pd.DataFrame:
    """
    ...
    if method == "straight_line":
        ...
        for year in range(1, useful_life + 1):
            ...
            straight_line_amount = (
                (book_value - salvage_value) / remaining_years
            ...
            if (
                straight_line_amount > declining_amount
                and book_value > salvage_value
            ):
                depreciation = straight_line_amount
            else:
                depreciation = min(
                    declining_amount, book_value - salvage_value
                )
        ...
        raise ValueError(
            f"Unknown method: {method}. Use 'straight_line', 'declining_balance', 'double_declining', or 'sum_of_years'"
        )
