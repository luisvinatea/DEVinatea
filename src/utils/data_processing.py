...
def clean_missing_values(df: pd.DataFrame, strategy: str = 'drop', fill_value: Any = None) -> pd.DataFrame:
    """
    ...
    if strategy == 'drop':
        ...
        raise ValueError(f"Unknown strategy: {strategy}. Use 'drop', 'mean', 'median', 'mode', or 'fill'")
        
    return df_cleaned


def detect_outliers(df: pd.DataFrame, method: str = 'iqr', columns: Optional[List[str]] = None, 
...
def encode_categorical(df: pd.DataFrame, columns: Optional[List[str]] = None, 
    ...
    if method == 'onehot':
        ...
        raise ValueError(f"Unknown method: {method}. Use 'onehot', 'label', or 'ordinal'")
